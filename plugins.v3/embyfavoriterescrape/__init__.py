...
from app.schemas import FileItem, WebhookEventInfo
...
class EmbyFavoriteRescrape(_PluginBase):
    ...
    def on_webhook_message(self, event: Event) -> None:
        ...
        if not self._is_favorite_removed(event_info):
            return
        logger.info(
            f"[EmbyFavoriteRescrape] 检测到移除收藏：{getattr(event_info, 'item_name', '') or ''} "
            f"({getattr(event_info, 'item_path', '') or ''})"
        )
        # 刮削耗时较长，放到后台线程执行，避免阻塞 Webhook 响应。
        threading.Thread(
            target=self.__dispatch_scrape,
            args=(event_info,),
    ...
    def _is_matched(self, event_info: WebhookEventInfo) -> bool:
        ...
        if self._exclude_keywords and any(
            ...
            logger.debug(f"[EmbyFavoriteRescrape] 命中排除关键词，跳过：{item_path}")
            return False
        # 类型过滤
    ...
    def _is_favorite_removed(self, event_info: WebhookEventInfo) -> bool:
        ...
        通用检测规则：收藏状态被解析为 ``False``（已移除），并且事件上下文确实是收藏
        相关事件（命中触发事件名或 SaveReason）。用事件上下文作为约束，避免把播放等
        非收藏事件的报文误判为移除收藏。
        """
    ...
    def _extract_favorite_state(self, event_info: WebhookEventInfo) -> Optional[bool]:
        ...
        if isinstance(favorite, bool):
            return favorite
        # 原始报文兜底（Emby 解析器不填充 item_favorite）
    ...
    def __dispatch_scrape(self, event_info: WebhookEventInfo) -> None:
        """后台线程入口，解析元数据并调用刮削链。"""
        try:
            self._rescrape(event_info)
        except Exception as err:
            logger.error(f"[EmbyFavoriteRescrape] 重新刮削失败：{err}")
            self.__notify(f"移除收藏自动刮削失败：{err}")

    def _rescrape(self, event_info: WebhookEventInfo) -> None:
        """识别媒体信息并对该媒体执行重新刮削。"""
        ...
        if not mtype or not item_path:
            logger.warn("[EmbyFavoriteRescrape] 缺少媒体路径或类型，跳过刮削")
            return

        # 媒体身份：优先使用统一身份，兼容旧事件通过 tmdb_id 回填。
        ...
        if not media_id:
            media_id = getattr(event_info, "tmdb_id", None)
            media_id = str(media_id).strip() if media_id is not None else None
            media_source = None if media_id is None else media_source
        ...
        logger.info(f"[EmbyFavoriteRescrape] 开始重新刮削：{item_path} ...")
        ok, message = ScrapingChain().scrape_metadata(
            fileitem=fileitem,
            mediainfo=mediainfo,
            overwrite=self._overwrite,
        )
        if ok:
            logger.info(f"[EmbyFavoriteRescrape] 刮削完成：{message}")
            self.__notify(f"移除收藏自动刮削完成：{message}")
        else:
            logger.warn(f"[EmbyFavoriteRescrape] 刮削未完成：{message}")

    def __notify(self, text: str) -> None:
        """按配置发送通知。"""
        if self._notify:
